...
public class MoveScreen extends TargetScreen {
	public MoveScreen(Combatant figure, Board board){
		super(figure, board, MeasureType.MANHATTAN);
	}
	
	@Override
	public Node[][] nodeMethod() {
		return Pathfinding.findPathableNodes(super.getFigure(), super.getBoard(), super.getFigure().getFigure().getMovement());
	}
	...
	public boolean renderNode(Node node) {
		return node.getWeight() > 0 && node.getWeight() <= super.getFigure().getFigure().getMovement();
	}
	...
	public int getMaxRange() {
		return this.getFigure().getFigure().getMovement();
	}
