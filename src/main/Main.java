...
public class Main extends BasicGame {
	...
	public void init(GameContainer arg0) throws SlickException {
		...
		try {
			party.add(new Figure("data/female.png", new SpriteSheet("data/dead.png", Main.TILE_HEIGHT, Main.TILE_HEIGHT).getSprite(2, 0)));
			party.add(new Figure("data/male.png", new SpriteSheet("data/dead.png", Main.TILE_HEIGHT, Main.TILE_HEIGHT).getSprite(2, 0)));
		} catch (SlickException e) {
			e.printStackTrace();
			System.exit(-1);
		}
		try {
			changeState(new PlayState(party));
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(-1);
		}
	...
	public void update(GameContainer gc, int delta) throws SlickException {
		...
		if(gc.getInput().isMousePressed(1)){
			screenStack.get(0).onRightClick();
		}else
			screenStack.get(0).update(gc, delta);
		for(Screen screen : screenStack)
			screen.updateBackground(gc, delta);
	}
